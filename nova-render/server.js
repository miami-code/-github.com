...
const server = http.createServer(async function(req, res) {
  ...
  if (req.method === "GET" && req.url.startsWith("/image?")) {
    ...
    var seed = params.get("seed") || "1";
    var pollinationsUrl = "https://image.pollinations.ai/prompt/" + encodeURIComponent(prompt) + "?width=1080&height=1080&seed=" + seed + "&model=flux&nologo=true";

    try {
      var imgResponse = await fetch(pollinationsUrl);
      if (!imgResponse.ok) throw new Error("Pollinations returned " + imgResponse.status);
