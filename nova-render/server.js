...
const server = http.createServer(async function(req, res) {
  ...
  // ── Anthropic API proxy ──
  // Test endpoint to debug image generation
  if (req.method === "GET" && req.url === "/imagetest") {
    try {
      var testUrl = "https://image.pollinations.ai/prompt/blue%20professional%20medical%20lab?width=200&height=200&nologo=true";
      console.log("Testing Pollinations connectivity...");
      var testRes = await fetch(testUrl, { signal: AbortSignal.timeout(20000) });
      console.log("Test response:", testRes.status, testRes.headers.get("content-type"));
